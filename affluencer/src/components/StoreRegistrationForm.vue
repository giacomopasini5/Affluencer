			...
			<v-col cols="10">
				<form @submit.prevent="handleSubmit" novalidate>
					<v-row>
						...
						<v-col cols="6">
							<v-text-field type="time" v-model="store.openTime" label="Orario di apertura" hide-details="auto" outlined dense :error="$v.store.openTime.$error"
							:rules="!$v.store.openTime.$error ? [] : [$v.store.openTime.required || 'L\'orario di apertura è obbligatorio']"></v-text-field>
						</v-col>
						<v-col cols="6">
							<v-text-field type="time" v-model="store.closeTime" label="Orario di chiusura" hide-details="auto" outlined dense :error="$v.store.closeTime.$error"
